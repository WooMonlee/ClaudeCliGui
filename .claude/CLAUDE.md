...
C# ASP.NET Core 8 实现的 Claude Code CLI Web GUI，自包含单文件发布（~46MB），放在 claude.exe 同目录即可使用。

## 项目结构
- `Program.cs` — 入口：Minimal API 路由、WebSocket、自动关机、项目 CRUD
- `ClaudeService.cs` — 核心：claude 子进程管理、流式 stdout/stderr 读取、WebSocket 广播、中文翻译
...
## 部署
复制 `claudeg.exe` 到 claude.exe 所在目录。启动后自动打开浏览器，关闭标签页后端自动退出。

## 开发日志
见 `.claude/changelog.md`
